...
import voiceFixerPlugin from './plugins/voice-fixer'; // Test plugin
...
class ErrorBoundary extends Component<{children: React.ReactNode}, {hasError: boolean, error: Error | null}> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-4 border border-red-500/50 bg-red-500/10 rounded-md text-red-500 text-sm">
          <h4 className="font-bold mb-1">Plugin Error</h4>
          <p className="font-mono text-xs opacity-80">{this.state.error?.message}</p>
          <button onClick={() => this.setState({hasError: false, error: null})} className="mt-2 text-xs underline">Dismiss</button>
        </div>
      );
    }
    ...
    <div className="flex flex-col gap-2">
      <div className="bg-app-card border border-app-border rounded p-4 shadow-sm relative group">
        ...
        <textarea
          ...
          onChange={(e) => {
            registry.api.document?.updateNodeContent(node.id, e.target.value);
            window.dispatchEvent(new CustomEvent('sandbox:nodeChanged', { detail: { id: node.id, content: e.target.value } }));
          }}
          onSelect={(e) => {
        ...
        {/* Inline Widgets */}
        {(registry as any).activeSelection?.nodeId === node.id && 
         (registry as any).activeSelection?.text && 
         (registry as any).inlineWidgets && 
         Object.entries((registry as any).inlineWidgets).length > 0 && (
          <div className="absolute -top-12 left-4 z-50 flex gap-2">
            {Object.entries((registry as any).inlineWidgets).map(([widgetId, WidgetComponent]: [string, any]) => (
              <WidgetComponent 
                key={widgetId}
                nodeId={node.id}
                text={(registry as any).activeSelection?.text}
                start={(registry as any).activeSelection?.start}
                end={(registry as any).activeSelection?.end}
              />
            ))}
          </div>
        )}
        ...
        <div className="ml-6 pl-4 border-l-2 border-app-border flex flex-col gap-4">
          {node.children.map(child => (
            <ErrorBoundary key={child.id}>
              <NodeView node={child} plugin={plugin} />
            </ErrorBoundary>
          ))}
        </div>
      )}
...
function App() {
  ...
  useEffect(() => {
    ...
    registry.register(voiceFixerPlugin);
  ...
  return (
    <div className="flex flex-col h-screen bg-app-bg text-app-text-primary">
      ...
      {/* Main workspace */}
      <main className="flex-1 overflow-auto p-8 flex flex-col items-center relative" id="sandbox-main">
        <div className="max-w-md w-full flex flex-col gap-4">
          {nodes.map(node => (
            <ErrorBoundary key={node.id}>
              <NodeView node={node} plugin={plugin} />
            </ErrorBoundary>
          ))}
