...
import voiceFixerPlugin from './plugins/voice-fixer';
...
function App() {
  ...
  return (
    <div className="flex flex-col h-screen bg-app-bg text-app-text-primary">
      ...
      {/* Render internal Toasts */}
      <div className="fixed bottom-4 right-4 flex flex-col gap-2 z-[999]">
        {registry.toasts.map(toast => (
          <div key={toast.id} className={`px-4 py-3 rounded-lg shadow-xl text-sm text-white ${toast.type === "success" ? "bg-green-600" : toast.type === "error" ? "bg-red-600" : "bg-blue-600"}`}>
            {toast.msg}
          </div>
        ))}
      </div>

      {/* Render internal Jobs inline briefly */}
      {registry.jobs.length > 0 && (
        <div className="fixed bottom-4 left-4 flex flex-col gap-2 z-[999]">
          {registry.jobs.map(job => (
            <div key={job.id} className="bg-app-card border border-app-border rounded-lg p-3 shadow-xl w-64">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-semibold truncate flex-1">{job.title}</span>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs text-app-text-secondary">{job.progress}%</span>
                  {job.onCancel && !job.completed && !job.error && (
                    <button 
                      onClick={() => registry.api.cancelJob?.(job.id)}
              ...
              <div className="h-1.5 w-full bg-app-input-bg rounded-full overflow-hidden mb-1">
                <div className={`h-full transition-all duration-300 ${job.error ? "bg-red-500" : job.completed ? "bg-green-500" : "bg-app-accent"}`} style={{ width: `${job.progress}%` }}></div>
              </div>
              <div className="text-xs text-app-text-muted truncate">
                {job.error ? job.error : (job.statusText || (job.completed ? "Completed" : "Processing..."))}
              </div>
            </div>
          ))}
        </div>
